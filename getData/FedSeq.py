...
def getPerIds(queue, ids):
    id_data_list = []
    id_labels = []
    for id in ids:
        id_data, id_label = getId(id)
        id_data_list.append(id_data)
        id_labels.append(id_label)
    queue.put((id_data_list, id_labels))

def getBatchData(map_ids:list, num_worker):
    ...
    for i in range(num_worker):
        ...
        getPerIds_pro = mp.Process(target=getPerIds, args=(perpared_batch_queue, per_pro_ids))
        getPerIds_pro.start()
...
def gen_seq(ids:list, batch_size:int, num_worker:int):
    ...
    for i in batch_amount:
        ...
        out_seq, label = getBatchData(map_ids, num_worker)
        yield(out_seq, label)
if __name__ == "__main__":
    ...
    ids = ids[:10]
    batch_data, _ = getBatchData(ids, 4)
